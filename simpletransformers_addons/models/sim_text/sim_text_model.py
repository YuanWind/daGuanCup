...
from simpletransformers_addons.models.sim_text.sim_text_utils import (
    InputExample,
...
from simpletransformers_addons.transformers.bert.modeling_nezha import NeZhaConfig
from simpletransformers_addons.models.sim_text.transformer_models.ensemble_model import EnsembleModelConfig, EnsembleModel
...
@dataclass
class SimTextArgs(ClassificationArgs):
    label_smooth: float = 0.0
    temperature: float = 0.0   # temperature>0,交叉熵regression
    wsl_alpha: float = 0.0     # wsl_alpha >0, wsl loss. wsl_alpha = 2.5较优值
    use_bimodel: bool = True
    use_symmetric: bool = False
    total_step_multipler: float = 1.0
    submodel_type: str = "bert"
    use_diff: bool = False
    use_swa: bool = False
    swa_lr: float = 2e-5
...
class SimTextModel:
    def __init__(
        ...
        **kwargs,
        ...
        MODEL_CLASSES = {
            ...
            "bert": (BertConfig, BertForSimText, BertTokenizer),
        ...
        elif isinstance(args, SimTextArgs):
            self.args = args

        # set symmetric
        if self.args.use_symmetric:
            self.args.use_bimodel = False
            if self.args.config:
                self.args.config.update({"segment_type": "relative"})
            else:
                self.args.config = {"segment_type": "relative"}
        ...
        if model_type == 'bert':
            config_class = SUBMODEL_MAP[self.args.submodel_type]
        if num_labels:
            ...
            if not self.args.quantized_model:
                if self.weight:
                    self.model = model_class.from_pretrained(
                        model_name, config=self.config, weight=torch.Tensor(self.weight).to(self.device), label_smooth=self.args.label_smooth,
                        temperature=self.args.temperature, wsl_alpha=self.args.wsl_alpha, use_bimodel=self.args.use_bimodel,
                        submodel_type=self.args.submodel_type, **kwargs,
                    )
                else:
                    self.model = model_class.from_pretrained(model_name, config=self.config, label_smooth=self.args.label_smooth,
                                                             temperature=self.args.temperature, wsl_alpha=self.args.wsl_alpha, use_bimodel=self.args.use_bimodel,
                                                             submodel_type=self.args.submodel_type, **kwargs)
            else:
                ...
                if self.weight:
                    self.model = model_class.from_pretrained(
                        ...
                        wsl_alpha=self.args.wsl_alpha,
                        use_bimodel=self.args.use_bimodel,
                        submodel_type=self.args.submodel_type
                    )
                else:
                    self.model = model_class.from_pretrained(None, config=self.config, state_dict=quantized_weights, label_smooth=self.args.label_smooth,
                                                             temperature=self.args.temperature, wsl_alpha=self.args.wsl_alpha, use_bimodel=self.args.use_bimodel,
                                                             submodel_type=self.args.submodel_type)
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if self.swa_model_valid:
            self.save_model(model=self.swa_model)
        else:
    ...
    def train(
        ...
        **kwargs,
        ...
        if args.optimizer == "AdamW":
            ...
            optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
            from simpletransformers_addons.optimizer import Lookahead
            optimizer = Lookahead(optimizer=optimizer, k=5, alpha=0.6)
        ...
        if args.scheduler == "constant_schedule":
            ...
            scheduler = get_linear_schedule_with_warmup(
                optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=int(t_total * args.total_step_multipler)
            )
            ...
            scheduler = get_cosine_schedule_with_warmup(
                ...
                num_training_steps=int(t_total * args.total_step_multipler),
        ...
        for _ in train_iterator:
            if max_run_time is not None:
                if timer.get_current_time() >= max_run_time:
                    logger.warning(f"run to max_time: {max_run_time}")
                    break
            model.train()
            if epochs_trained > 0:
                epochs_trained -= 1
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.fp16:
                        ...
                        optimizer.step()
                    if self.args.use_swa and (global_step + 1) > self.args.swa_start_step:
                        if (global_step + 1) % self.args.swa_steps == 0:
                    ...
                    if args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        if self.swa_model_valid:
                            self.save_model(output_dir_current, optimizer, scheduler, model=self.swa_model)
                        else:
                    ...
                    if args.evaluate_during_training and (
                        ...
                        if args.save_eval_checkpoints:
                            if self.swa_model_valid:
                                self.save_model(output_dir_current, optimizer, scheduler, model=self.swa_model, results=results)
                            else:
                                self.save_model(output_dir_current, optimizer, scheduler, model=model,
                                                results=results)
                        ...
                        if not best_eval_metric:
                            ...
                            if self.swa_model_valid:
                                self.save_model(args.best_model_dir, optimizer, scheduler, model=self.swa_model, results=results)
                            else:
                                self.save_model(args.best_model_dir, optimizer, scheduler, model=model,
                                                results=results)
                        if best_eval_metric and args.early_stopping_metric_minimize:
                            if best_eval_metric - results[args.early_stopping_metric] > args.early_stopping_delta:
                                ...
                                if self.swa_model_valid:
                                    self.save_model(
                                        args.best_model_dir, optimizer, scheduler, model=self.swa_model, results=results
                                    )
                                else:
                                    self.save_model(
                                ...
                                if args.use_early_stopping:
                                    if early_stopping_counter < args.early_stopping_patience:
                                        ...
                                        if args.wandb_project or self.is_sweeping:
                                            wandb.log({args.early_stopping_metric: best_eval_metric})
                                        return (
                                            global_step,
                                            tr_loss / global_step
    ...
    def load_and_cache_examples(
        ...
        if os.path.exists(cached_features_file) and (
            ...
            if self.args.use_diff:
                for f in features:
                    for k, v in f.items():
                        f[k].segment_ids = get_diff_token_type_ids(
                            input_ids=f[k].input_ids,
                            token_type_ids=f[k].segment_ids,
                            attention_mask=f[k].input_mask
                        )
        ...
        if not args.use_symmetric:
            ...
            if output_mode == "classification":
                ...
                all_label_ids = torch.tensor([f['feature_a'].label_id for f in features], dtype=torch.float)
            ...
            all_input_mask_a = torch.tensor([f['feature_a']["input_mask"] for f in features], dtype=torch.long)
            all_segment_ids_a = torch.tensor([f['feature_a']["segment_ids"] for f in features], dtype=torch.long)
            all_position_ids_a = torch.tensor([f['feature_a']["position_ids"] for f in features], dtype=torch.long)
            all_input_ids_b = torch.tensor([f['feature_b']["input_ids"] for f in features], dtype=torch.long)
            all_input_mask_b = torch.tensor([f['feature_b']["input_mask"] for f in features], dtype=torch.long)
            all_segment_ids_b = torch.tensor([f['feature_b']["segment_ids"] for f in features], dtype=torch.long)
            all_position_ids_b = torch.tensor([f['feature_b']["position_ids"] for f in features], dtype=torch.long)
            ...
            dataset = TensorDataset(all_input_ids_a, all_input_mask_a, all_segment_ids_a, all_label_ids,
                                            all_input_ids_b, all_input_mask_b, all_segment_ids_b,
                                    all_position_ids_a, all_position_ids_b)
    ...
    def predict(self, to_predict, multi_label=False, no_cache=True):
        """
        ...
        if not multi_label and self.args.onnx:
            ...
            if args.sliding_window:
                eval_dataset, window_counts = self.load_and_cache_examples(eval_examples, evaluate=True, no_cache=no_cache)
                preds = np.empty((len(eval_dataset), self.num_labels))
                if multi_label:
                    out_label_ids = np.empty((len(eval_dataset), self.num_labels))
                else:
                ...
                eval_dataset = self.load_and_cache_examples(
                    eval_examples, evaluate=True, multi_label=multi_label, no_cache=no_cache
                )
    ...
    def _move_model_to_device(self):
        ...
        if isinstance(self.model.bert, EnsembleModel):
            self.model.bert.move_model_to_device()
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            if self.args.use_symmetric:
                inputs["position_ids_a"] = batch[7]
                inputs["position_ids_b"] = batch[8]
