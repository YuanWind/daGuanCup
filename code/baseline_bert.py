...
from DaGuanCup.code.model import tokenization
...
checkpoint_path = '../models/roberta_zh_l12/bert_model.ckpt'
...
def random_mask(text_ids):
    ...
    for r, i in zip(rands, text_ids):
        if r < 0.15 * 0.8:
            ...
            input_ids.append(np.random.choice(len(tokens)) + 5)
            output_ids.append(i)
        else:
            input_ids.append(i)
            output_ids.append(0)
...
def sample_convert(text1, random=False):
    ...
    tokens = tokenizer.tokenize(" ".join(text1))
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(epoch)
            model.save_weights('../models/result/best_model_{}.weights'.format(epoch))
