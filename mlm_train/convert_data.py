...
def get_kfold_data(input_data_file,
    ...
    with open(input_data_file, encoding='utf-8') as f:
        for line in tqdm(f):
            record = json.loads(line)
            sid_a = meta_info['sentence2sid'][record['text']]
    ...
    for split_id in tqdm(range(n_splits)):
        ...
        with open(os.path.join(output_dir, str(split_id), 'train.jsonl'), 'w', encoding='utf-8') as fout:
            for record in tqdm(train_records):
                fout.write(json.dumps(record, ensure_ascii=False) + '\n')
        with open(os.path.join(output_dir, str(split_id), 'dev.jsonl'), 'w', encoding='utf-8') as fout:
        ...
        # with open(os.path.join(output_dir, str(split_id), 'train_enhanced.jsonl'), 'w', encoding='utf-8') as fout:
...
def construct_mlm_data(train_data_file,
                       ...
                       random_seed=9527):
    random.seed(random_seed)
    with open(output_data_file,'w',encoding='utf-8') as f_w:
...
#     construct_mlm_data(os.path.join(train_file + '.str'),
#                        os.path.join(test_file + '.str'),
#                        os.path.join(output_dir, 'train_mlm.txt'),
#                        random_seed=random_seed)
